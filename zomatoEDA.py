...
st.title(" :chart_with_downwards_trend: :chart_with_upwards_trend: EDA and Predictive Analysis of Restaurant :hotel: Data")
df = read_file()

#Overview of data
st.header("Overview :bar_chart: of our Restaurant Data")
...
st.header(" :1234: Data Cleaning and Preproccessing")
st.subheader("Count of Null Values in each columns")
nullValuesDf = df.isnull().sum()
st.table(nullValuesDf)

st.subheader("Percentage of Null Values :eyes: in each columns")
...
st.subheader(" :1234: Average Restaurant Rating")
...
st.subheader("Most Famous Restaurant Chains :first_place_medal: :second_place_medal: :third_place_medal:")
...
#types of Restaurant
st.subheader("Famous Restaurants based upon food :shallow_pan_of_food: offered")
resTypes = typesOfRestaurants()
st.plotly_chart([resTypes])

#highest rated restaurants
st.subheader("Top Restaurants :hotel: Based Upon Votes")
...
st.subheader("Most Popular Cuisines :stuffed_flatbread: :pancakes: :takeout_box: ")
...
st.subheader("Approx Two People Cost :credit_card: Distribution")
...
st.subheader(" :1234: Cost of Two vs Rating Analysis")
X_values = st.slider(
...
st.subheader("List of Most Expensive :moneybag: Restaurants")
st.table(mostExpensiveRestList())

st.subheader("List of Most Cheap :dollar: Restaurants")
def cheapRestList():
    return data['approx_cost(for two people)'].nsmallest(24).index.unique()
st.table(cheapRestList())

st.subheader("Finding Best Budgets Restaurants     :100:")
...
st.subheader("Finding Best budget Restaurants in any location    :100:")
...
st.subheader("Geolocations & HeatMaps :fire: of Restaurants")
try:
    locations=pd.DataFrame({"Name":df['location'].unique()})
    #locations.drop(locations[locations['Name']=="Banashankari"].index, inplace = True)
...
#for plotting geolocations and basemap+heatmap
    ...
    st.write(" :man-facepalming: :face_palm: :woman-facepalming: Nominatium and GeoCoder Service are not available")
...
st.subheader(" :poultry_leg: Most Liked Dishes  :takeout_box: across different types of Restaurants")
